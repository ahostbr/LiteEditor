...
Use this when you want to open LiteEditor from another device (phone, tablet, another laptop).

## CLI ↔ Env option map

The LiteEditor CLI accepts the following configuration options, available either as CLI flags or environment variables:

| CLI flag                | Env var                  | Notes                              |
| ----------------------- | ------------------------ | ---------------------------------- |
| `--mode <web\|desktop>` | `LITEEDITOR_MODE`        | Runtime mode.                      |
| `--port <number>`       | `LITEEDITOR_PORT`        | HTTP/WebSocket port.               |
| `--host <address>`      | `LITEEDITOR_HOST`        | Bind interface/address.            |
| `--base-dir <path>`     | `LITEEDITOR_HOME`        | Base directory.                    |
| `--dev-url <url>`       | `VITE_DEV_SERVER_URL`    | Dev web URL redirect/proxy target. |
| `--no-browser`          | `LITEEDITOR_NO_BROWSER`  | Disable auto-open browser.         |
| `--auth-token <token>`  | `LITEEDITOR_AUTH_TOKEN`  | WebSocket auth token.              |
