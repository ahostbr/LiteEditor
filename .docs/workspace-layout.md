...
- `/apps/desktop`: Electron shell. Spawns a desktop-scoped `liteeditor` backend process and loads the shared web app.
...
- `/packages/shared`: Shared runtime utilities consumed by both server and web. Uses explicit subpath exports (e.g. `@liteeditor/shared/git`, `@liteeditor/shared/DrainableWorker`) — no barrel index.
