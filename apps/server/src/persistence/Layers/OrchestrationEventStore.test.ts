import { CommandId, EventId, MessageId, ProjectId, ThreadId } from "@liteeditor/contracts";
...
layer("OrchestrationEventStore", (it) => {
  ...
  it.effect("normalizes legacy claude providers when replaying stored events", () =>
    Effect.gen(function* () {
      const eventStore = yield* OrchestrationEventStore;
      const sql = yield* SqlClient.SqlClient;
      const now = new Date().toISOString();
      ...
      yield* sql`
        ...
        VALUES (
          ...
          ${JSON.stringify({
            threadId,
            messageId: MessageId.makeUnsafe("message-legacy-claude"),
            provider: "claude",
            createdAt: now,
          })},
          ${"{}"}
      ...
      if (replayed[0]?.type === "thread.turn-start-requested") {
        assert.equal(replayed[0].payload.provider, "claudeAgent");
      }
    }),
  );
});
