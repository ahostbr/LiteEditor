...
import { ThreadId } from "@liteeditor/contracts";
...
it.layer(makeDirectoryLayer(SqlitePersistenceMemory))("ProviderSessionDirectoryLive", (it) => {
  ...
  it("normalizes legacy claude runtime rows when reading persisted bindings", () =>
    Effect.gen(function* () {
      const directory = yield* ProviderSessionDirectory;
      const runtimeRepository = yield* ProviderSessionRuntimeRepository;
      ...
      if (Option.isSome(runtime)) {
        ...
        assert.equal(runtime.value.adapterKey, "claude");
      }
      ...
      if (Option.isSome(binding)) {
        ...
        assert.equal(binding.value.adapterKey, "claude");
      }
    }));

  it("rehydrates persisted mappings across layer restart", () =>
    Effect.gen(function* () {
      const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "liteeditor-provider-directory-"));
