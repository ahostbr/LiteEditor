...
import type { OrchestrationReadModel } from "@liteeditor/contracts";
...
import { CliConfig, recordStartupHeartbeat, liteEditorCli, type CliConfigShape } from "./main";
...
const testLayer = Layer.mergeAll(
  Layer.succeed(CliConfig, {
    cwd: "/tmp/liteeditor-test-workspace",
    fixPath: Effect.void,
...
const runCli = (
  ...
  env: Record<string, string> = { LITEEDITOR_NO_BROWSER: "true" },
) => {
  return Command.runWith(liteEditorCli, { version: "0.0.0-test" })(args).pipe(
    Effect.provide(
...
it.layer(testLayer)("server CLI command", (it) => {
  it.effect("parses all CLI flags and wires scoped start/stop", () =>
    Effect.gen(function* () {
      ...
      assert.equal(resolvedConfig?.baseDir, "/tmp/liteeditor-cli-home");
      assert.equal(resolvedConfig?.stateDir, "/tmp/liteeditor-cli-home/dev");
  ...
  it.effect("uses env fallbacks when flags are not provided", () =>
    Effect.gen(function* () {
      yield* runCli([], {
        ...
        LITEEDITOR_PORT: "4999",
        LITEEDITOR_HOST: "100.88.10.4",
        LITEEDITOR_HOME: "/tmp/liteeditor-env-home",
        VITE_DEV_SERVER_URL: "http://localhost:5173",
        LITEEDITOR_NO_BROWSER: "true",
        LITEEDITOR_AUTH_TOKEN: "env-token",
      });
      ...
      assert.equal(resolvedConfig?.baseDir, "/tmp/liteeditor-env-home");
      assert.equal(resolvedConfig?.stateDir, "/tmp/liteeditor-env-home/dev");
  ...
  it.effect("prefers --mode over LITEEDITOR_MODE", () =>
    Effect.gen(function* () {
  ...
  it.effect("prefers --no-browser over LITEEDITOR_NO_BROWSER", () =>
    Effect.gen(function* () {
      yield* runCli(["--no-browser"], {
        LITEEDITOR_NO_BROWSER: "false",
      });
  ...
  it.effect("supports CLI and env for bootstrap/log websocket toggles", () =>
    Effect.gen(function* () {
      yield* runCli(["--auto-bootstrap-project-from-cwd"], {
        LITEEDITOR_MODE: "desktop",
        LITEEDITOR_LOG_WS_EVENTS: "false",
        LITEEDITOR_AUTO_BOOTSTRAP_PROJECT_FROM_CWD: "false",
        LITEEDITOR_NO_BROWSER: "true",
      });
