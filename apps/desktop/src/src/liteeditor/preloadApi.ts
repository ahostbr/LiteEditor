...
const api = {
  ...
  fs: {
    ...
    ensureDir: (path: string): Promise<void> => ipcRenderer.invoke("fs:ensure-dir", path),
    deleteFile: (path: string): Promise<void> => ipcRenderer.invoke("fs:delete-file", path),
    readTree: (root: string, depth?: number): Promise<unknown[]> =>
      ipcRenderer.invoke("fs:read-tree", root, depth),
    readDir: (dirPath: string): Promise<unknown[]> => ipcRenderer.invoke("fs:read-dir", dirPath),
    watchStart: (path: string): void => ipcRenderer.send("fs:watch-start", path),
    watchStop: (): void => ipcRenderer.send("fs:watch-stop"),
    watchDir: (path: string): void => ipcRenderer.send("fs:watch-dir", path),
    unwatchDir: (path: string): void => ipcRenderer.send("fs:unwatch-dir", path),
    ...
    showInExplorer: (path: string): void => ipcRenderer.send("shell:open-path", path),
  },
  ...
  git: {
    ...
    diffCached: (path: string): Promise<string> => ipcRenderer.invoke("git:diff-cached", path),
    ...
    unstageAll: (): Promise<void> => ipcRenderer.invoke("git:unstage-all"),
    commit: (summary: string, description?: string): Promise<void> =>
      ipcRenderer.invoke("git:commit", summary, description),
    push: (): Promise<unknown> => ipcRenderer.invoke("git:push"),
    ...
    currentBranch: (): Promise<unknown> => ipcRenderer.invoke("git:current-branch"),
    ...
    statusPorcelain: (): Promise<string> => ipcRenderer.invoke("git:status-porcelain"),
    worktreeList: (): Promise<unknown[]> => ipcRenderer.invoke("git:worktree-list"),
    ...
    branchList: (): Promise<unknown[]> => ipcRenderer.invoke("git:branch-list"),
  ...
  search: {
    setRoot: (root: string): Promise<void> => ipcRenderer.invoke("search:set-root", root),
  ...
  browser: {
    ...
    destroyView: (sessionId: string): void => ipcRenderer.send("browser:destroy-view", sessionId),
    setBounds: (sessionId: string, bounds: NativeViewBounds): void =>
      ipcRenderer.send("browser:set-bounds", sessionId, bounds),
    showView: (sessionId: string): void => ipcRenderer.send("browser:show-view", sessionId),
    hideView: (sessionId: string): void => ipcRenderer.send("browser:hide-view", sessionId),
    onStateUpdate: (
      callback: (
        event: unknown,
        data: {
    ...
    stop: (sessionId: string): Promise<unknown> => ipcRenderer.invoke("browser:stop", sessionId),
    // Agent tools
    ...
    listSessions: (): Promise<string[]> => ipcRenderer.invoke("browser:list-sessions"),
  },

  claude: {
    createSession: (): Promise<string> => ipcRenderer.invoke("claude:create-session"),
    destroySession: (sessionId: string): void =>
      ipcRenderer.send("claude:destroy-session", sessionId),
    setBounds: (sessionId: string, bounds: NativeViewBounds): void =>
      ipcRenderer.send("claude:set-bounds", sessionId, bounds),
    showView: (sessionId: string): void => ipcRenderer.send("claude:show-view", sessionId),
    hideView: (sessionId: string): void => ipcRenderer.send("claude:hide-view", sessionId),
    onHostOp: (
      callback: (request: { id: string; op: string; payload?: Record<string, unknown> }) => void,
    ): (() => void) => {
      const handler = (
        _e: unknown,
        request: { id: string; op: string; payload?: Record<string, unknown> },
      ) => callback(request);
      ipcRenderer.on("claude:host-op", handler);
      return () => ipcRenderer.removeListener("claude:host-op", handler);
    },
    sendHostOpResult: (result: {
      id: string;
      ok: boolean;
      payload?: Record<string, unknown>;
      error?: string;
    }): void => ipcRenderer.send("claude:host-op-result", result),
  },

  codex: {
    createSession: (): Promise<string> => ipcRenderer.invoke("codex:create-session"),
    destroySession: (sessionId: string): void =>
    ...
    showView: (sessionId: string): void => ipcRenderer.send("codex:show-view", sessionId),
    hideView: (sessionId: string): void => ipcRenderer.send("codex:hide-view", sessionId),
  },
  ...
  scripts: {
    ...
    stop: (sessionId: string): Promise<void> => ipcRenderer.invoke("scripts:stop", sessionId),
    running: (): Promise<unknown[]> => ipcRenderer.invoke("scripts:running"),
  },
  ...
  settings: {
    ...
    save: (data: string): Promise<void> => ipcRenderer.invoke("settings:save", data),
  },

  integrations: {
    listStatus: (): Promise<IntegrationStatus[]> => ipcRenderer.invoke("integrations:list-status"),
  ...
  github: {
    checkCli: (): Promise<unknown> => ipcRenderer.invoke("github:check-cli"),
    installCli: (): Promise<unknown> => ipcRenderer.invoke("github:install-cli"),
    setCwd: (cwd: string): Promise<void> => ipcRenderer.invoke("github:set-cwd", cwd),
    repoInfo: (): Promise<unknown> => ipcRenderer.invoke("github:repo-info"),
    prList: (state?: string): Promise<unknown[]> => ipcRenderer.invoke("github:pr-list", state),
    prGet: (number: number): Promise<unknown> => ipcRenderer.invoke("github:pr-get", number),
    prDiff: (number: number): Promise<string> => ipcRenderer.invoke("github:pr-diff", number),
    prCreate: (
      title: string,
      body: string,
      base: string,
    ...
    prClose: (number: number): Promise<string> => ipcRenderer.invoke("github:pr-close", number),
    ...
    issueGet: (number: number): Promise<unknown> => ipcRenderer.invoke("github:issue-get", number),
    issueCreate: (
      title: string,
      body: string,
      labels?: string[],
    ...
    labelsList: (): Promise<unknown[]> => ipcRenderer.invoke("github:labels-list"),
    collaboratorsList: (): Promise<string[]> => ipcRenderer.invoke("github:collaborators-list"),
    prTemplate: (): Promise<string | null> => ipcRenderer.invoke("github:pr-template"),
  },
  ...
  workspace: {
    ...
    save: (data: string): Promise<void> => ipcRenderer.invoke("workspace:save", data),
  ...
  dialog: {
    ...
    showMessageBox: (options: {
      ...
      defaultId?: number;
      cancelId?: number;
    }): Promise<number> => ipcRenderer.invoke("dialog:show-message-box", options),
  },
