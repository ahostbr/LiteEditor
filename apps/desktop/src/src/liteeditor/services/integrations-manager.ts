...
export class IntegrationsManager extends EventEmitter {
  ...
  private readonly runtimeStates = new Map<
    IntegrationId,
    { state: IntegrationState; message?: string }
  >();
  ...
  async checkUpdates(targetId?: IntegrationId): Promise<IntegrationStatus[]> {
    ...
    for (const id of ids) {
      ...
      try {
        ...
        this.appendIntegrationLog(
          id,
          `Update check succeeded. Latest Marketplace version: ${manifest.version}.`,
        );
      } catch (err) {
        const cacheEntry = this.ensureCacheEntry(id);
        cacheEntry.lastCheckedAt = Date.now();
    ...
    return this.runExclusive(id, async () => {
      ...
      try {
        ...
        this.emitProgress({
          id,
          stage: "downloading",
          message: `Downloading ${manifest.extensionId}@${manifest.version}...`,
          percent: 0,
        });
        ...
        if (manifest.sha256) {
          if (!this.matchesHash(manifest.sha256, archiveHashHex, archiveHashBase64)) {
            throw new Error(
              "SHA256 mismatch: downloaded archive hash does not match Marketplace metadata.",
            );
          }
        } else {
          this.emitProgress({
            ...
            stage: "verifying",
            message:
              "Marketplace did not provide SHA256; validating extension identity and runtime files instead.",
          });
        }
        ...
        this.emitProgress({
          id,
          stage: "installing",
          message: "Installing managed extension files...",
        });
        const finalPath = this.installManagedPayload(
          id,
        ...
        this.emitProgress({
          id,
          stage: "done",
          message: "Install completed successfully. Reopen the panel to load the new version.",
        });
        this.runtimeStates.delete(id);
        return this.getStatus(id);
      } catch (err) {
      ...
      if (validation.ok) {
        return {
          valid: {
            ...
            path: fullPath,
            source: "external",
            version: validation.version ?? folderName.slice(prefix.length),
          },
          broken: null,
        };
      }
  ...
  private validateCandidate(
    ...
    id: IntegrationId,
    source: "managed" | "external",
  ): { ok: boolean; version: string | null; message: string } {
    try {
  ...
  private validateManagedInstallRecord(
    id: IntegrationId,
    rootPath: string,
    ...
    for (const requiredFile of spec.requiredFiles) {
      ...
      if (!expectedHash) {
        return {
          ok: false,
          message: `Managed install record is missing hash for ${requiredFile}.`,
        };
      }
      ...
      if (!actualHash || actualHash.toLowerCase() !== expectedHash.toLowerCase()) {
        return {
          ok: false,
          message: `Managed install integrity check failed for ${requiredFile}.`,
        };
      }
    }
  ...
  private readIdentity(
    rootPath: string,
  ): { publisher: string; name: string; version: string | null } | null {
    ...
    try {
      ...
      const publisher =
        typeof parsed.publisher === "string" ? parsed.publisher.trim().toLowerCase() : "";
  ...
  private readAndValidateIdentity(rootPath: string, id: IntegrationId): { version: string | null } {
    ...
    if (identity.publisher !== spec.publisher || identity.name !== spec.name) {
      throw new Error(
        `Identity mismatch. Expected ${spec.extensionId}, got ${identity.publisher}.${identity.name}`,
      );
    }
    ...
    nodeReadable.on("data", (chunk: Buffer | string) => {
      ...
      if (total > 0) {
        ...
        this.emitProgress({
          ...
          stage: "downloading",
          percent,
          message: "Downloading VSIX package...",
        });
      }
    });
  ...
  private installManagedPayload(id: IntegrationId, version: string, sourcePath: string): string {
    ...
    try {
      rmSync(tempInstallPath, { recursive: true, force: true });
    } catch {
      /* ignore */
    }
    ...
    for (const existing of this.getVersionDirectories(integrationRoot)) {
      ...
      try {
        rmSync(join(integrationRoot, existing), { recursive: true, force: true });
      } catch {
        /* ignore */
      }
    }
  ...
  private writeInstallRecord(rootPath: string, record: InstallRecord): void {
    writeFileSync(
      join(rootPath, ".liteeditor-install.json"),
