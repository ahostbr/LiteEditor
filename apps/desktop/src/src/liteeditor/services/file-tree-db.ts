...
export class FileTreeDB {
  ...
  constructor() {
    ...
    this.stmtIsScanned = this.db.prepare("SELECT scanned FROM entries WHERE path = ?");
    this.stmtMarkScanned = this.db.prepare("UPDATE entries SET scanned = 1 WHERE path = ?");
    this.stmtInvalidateDir = this.db.prepare("UPDATE entries SET scanned = 0 WHERE path = ?");
    this.stmtDeleteByParent = this.db.prepare("DELETE FROM entries WHERE parent = ?");
    this.stmtDeleteEntry = this.db.prepare("DELETE FROM entries WHERE path = ?");
  }
