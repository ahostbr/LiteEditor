...
type RendererHostOpInvoker = (
  op: string,
  payload?: Record<string, unknown>,
) => Promise<RendererHostOpResult>;
...
export class ClaudeBridge {
  ...
  async requestToolPermission(
    ...
    if (!wc) {
      return {
        ...
        result: { behavior: "deny", message: "Session is not available", interrupt: false },
      };
    }
    ...
    try {
      ...
      return {
        type: "tool_permission_response",
        result: { behavior: "deny", message: errorText, interrupt: false },
      };
    }
  }
  ...
  private handleCancelRequest(message: any): void {
    const targetRequestId =
      typeof message?.targetRequestId === "string" ? message.targetRequestId : "";
  ...
  private async handleRequest(sessionId: string, wc: WebContents, message: any): Promise<void> {
    ...
    try {
      ...
      if (abortController.signal.aborted) {
        console.warn(`[claude-bridge] Request '${requestType}' was cancelled`, {
          requestId,
          channelId,
        });
        return;
  ...
  private async handleKnownRequest(
    ...
    switch (requestType) {
      ...
      case "set_thinking_level": {
        const thinkingLevel =
          typeof request.thinkingLevel === "string" ? request.thinkingLevel : null;
      ...
      case "check_git_status": {
        ...
        return Object.keys(result).length > 0
          ? result
          : { isClean: true, hasChanges: false, branch: null };
      }
      case "checkout_branch": {
        const result = await this.invokeRendererHostOp("checkout_branch", request);
        return Object.keys(result).length > 0
          ? result
          : { status: "failed", branch: request.branch ?? null };
      }
  ...
  private buildClaudeConfig(): Record<string, unknown> {
    ...
    return {
      ...
      models: [
        ...
        {
          value: "claude-sonnet-4-6-max-1m",
  ...
  private async invokeRendererHostOp(
    op: string,
    payload: Record<string, unknown> = {},
  ): Promise<RendererHostOpResult> {
  ...
  private async handleShowNotification(
    ...
    const type: Electron.MessageBoxOptions["type"] =
  ...
  private async handleOpenTerminal(
    ...
    const terminalName =
      typeof request.terminalName === "string" ? request.terminalName : undefined;
  ...
  private async handleOpenClaudeInTerminal(
    request: Record<string, unknown>,
  ): Promise<Record<string, unknown>> {
  ...
  private async handleExecInTerminal(
    request: Record<string, unknown>,
  ): Promise<Record<string, unknown>> {
  ...
  private async handleGetTerminalContents(
    request: Record<string, unknown>,
  ): Promise<Record<string, unknown>> {
