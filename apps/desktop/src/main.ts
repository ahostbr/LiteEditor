...
} from "./liteeditor/registerLiteEditorDesktop";
...
const APP_USER_MODEL_ID = "com.litesuite.liteeditor";
const USER_DATA_DIR_NAME = isDevelopment ? "liteeditor-dev" : "liteeditor";
const LEGACY_USER_DATA_DIR_NAME = isDevelopment ? "LiteEditor (Dev)" : "LiteEditor (Alpha)";
...
function handleFatalStartupError(stage: string, error: unknown): void {
  ...
  if (!isQuitting) {
    isQuitting = true;
    dialog.showErrorBox("LiteEditor failed to start", `Stage: ${stage}\n${message}${detail}`);
  }
  stopBackend();
  restoreStdIoCapture?.();
  app.quit();
...
async function checkForUpdatesFromMenu(): Promise<void> {
  ...
  if (updateState.status === "up-to-date") {
    void dialog.showMessageBox({
      ...
      message: `LiteEditor ${updateState.currentVersion} is currently the newest version available.`,
      buttons: ["OK"],
    });
...
function createWindow(): BrowserWindow {
  ...
  registerLiteEditorDesktop(window, {
    appDescription: "LiteEditor — spatial coding with AI agent orchestration.",
    appDisplayName: APP_DISPLAY_NAME,
    commitHash: resolveAboutCommitHash(),
    iconPath:
      resolveIconPath(process.platform === "win32" ? "ico" : "png") ?? resolveIconPath("png"),
  });
