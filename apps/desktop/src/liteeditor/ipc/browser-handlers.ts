...
export function registerBrowserHandlers(mainWindow: BrowserWindow): void {
  ...
  ipcMain.handle("browser:create-view", async (_e, initialUrl: string) => {
    ...
    return browserManager.createView(owner, initialUrl);
  });
  ...
  ipcMain.handle(
    "browser:scroll",
    async (_e, sessionId: string, direction: "up" | "down" | "left" | "right", amount: number) => {
      try {
        const wc = browserManager.getWebContents(sessionId);
        if (!wc) return { success: false, error: "Session not found" };
  ...
  ipcMain.handle(
    ...
    async (_e, sessionId: string, elementIndex: number, optionIndex: number) => {
      try {
        const wc = browserManager.getWebContents(sessionId);
        if (!wc) return { success: false, error: "Session not found" };
