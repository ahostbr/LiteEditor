...
function exec(cmd: string, args: string[], cwd: string): Promise<string> {
  return new Promise((resolve, reject) => {
    execFile(
      cmd,
      args,
      { cwd, maxBuffer: 10 * 1024 * 1024, timeout: 30000 },
      (err, stdout, stderr) => {
        if (err) {
...
export class GitHubService {
  ...
  async prList(state: string = "open", limit: number = 50): Promise<unknown[]> {
    ...
    const args = [
      ...
      "list",
      "--json",
      "number,title,body,state,author,labels,reviewDecision,headRefName,baseRefName,additions,deletions,changedFiles,isDraft,mergeable,updatedAt,createdAt,comments,reviewRequests,url",
      "--limit",
      String(limit),
    ];
  ...
  async issueList(state: string = "open", limit: number = 50): Promise<unknown[]> {
    const args = [
      ...
      "list",
      "--json",
      "number,title,body,state,author,labels,assignees,milestone,comments,createdAt,updatedAt,url",
      "--limit",
      String(limit),
    ];
  ...
  async prTemplate(): Promise<string | null> {
    try {
      const template = await readFile(
        join(this.cwd, ".github", "pull_request_template.md"),
        "utf-8",
      );
      return template;
    } catch {
