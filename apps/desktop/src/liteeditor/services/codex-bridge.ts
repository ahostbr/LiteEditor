...
export class CodexBridge {
  ...
  private workspaceRootOptions: { roots: string[]; labels: Record<string, string> } = {
    roots: [],
    labels: {},
  };
    ...
    switch (msgType) {
      ...
      case "electron-update-workspace-root-options": {
        const roots = Array.isArray(message.roots)
          ...
          : [];
        const labels =
          message.labels && typeof message.labels === "object"
      ...
      case "electron-add-new-workspace-root-option": {
        ...
        this.sendToWebview(wc, {
          type: "electron-add-new-workspace-root-option-result",
          success: true,
        });
        break;
      }
  ...
  private async handleMcpRequest(wc: WebContents, message: any): Promise<void> {
    ...
    if (!this.proc) {
      ...
      this.sendToWebview(wc, {
        ...
        message: {
          id,
          error: { code: -1, message: "codex.exe server exited during initialization" },
        },
      });
      return;
    }
  ...
  private getInternalUrlResponse(url: string, body: string | undefined): InternalUrlResponse {
    ...
    switch (path) {
      ...
      case "get-configuration":
        return {
          status: 200,
          body: { value: stateKey ? (this.configurationState.get(stateKey) ?? null) : null },
        };
      ...
      case "get-global-state":
        return {
          status: 200,
          body: { value: stateKey ? (this.globalState.get(stateKey) ?? null) : null },
        };
  ...
  private buildOpenTargetsResponse(parsedBody: Record<string, unknown>): {
    ...
    const roots = this.getWorkspaceRoots();
    const targetIds = cwd && !roots.includes(cwd) ? [cwd, ...roots] : roots;
  ...
  private persistLiteEditorProjectRoot(root: string): void {
    ...
    try {
      ...
      if (existsSync(workspacePath)) {
        try {
          const parsed = JSON.parse(readFileSync(workspacePath, "utf-8")) as Record<
            string,
            unknown
          >;
