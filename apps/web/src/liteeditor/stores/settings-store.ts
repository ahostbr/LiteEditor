...
export const useSettingsStore = create<SettingsState>((set, get) => ({
  ...
  saveSettings: async () => {
    ...
    try {
      ...
      logError("settings", "Failed to save settings", err);
    }
  },

  loadWorkspaceSettings: async (projectRoot: string) => {
    try {
      const parsed = (await window.api.workspace.loadSettings(projectRoot)) as Record<
        string,
        unknown
      > | null;
      if (parsed && typeof parsed === "object") {
  ...
  setWorkspaceSetting: (key: string, value: unknown) => {
    ...
    window.api.workspace
      .saveSettings(projectRoot, JSON.stringify(newOverrides, null, 2))
      .catch(() => {});
  },
