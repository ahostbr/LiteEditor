...
export const useGitStore = create<GitState>((set, get) => ({
  ...
  toggleStaged: async (path) => {
    ...
    try {
      ...
      logError("git", "Failed to toggle staged state", err);
    }
  },
  ...
  stageAll: async () => {
    try {
      ...
      logError("git", "Failed to stage all files", err);
    }
  },
  ...
  unstageAll: async () => {
    try {
      ...
      logError("git", "Failed to unstage all files", err);
    }
  },
  ...
  createCommit: async () => {
    ...
    try {
      ...
      logError("git", "Failed to create commit", err);
    }
  },
  ...
  pushOrigin: async () => {
    ...
    try {
      ...
      logError("git", "Failed to push to origin", err);
    } finally {
      set({ isPushing: false });
    }
  },
  ...
  pullOrigin: async () => {
    ...
    try {
      ...
      logError("git", "Failed to pull from origin", err);
    } finally {
      set({ isPulling: false });
    }
  },
  ...
  fetchOrigin: async () => {
    ...
    try {
      ...
      logError("git", "Failed to fetch from origin", err);
    } finally {
  ...
  loadBranches: async () => {
    try {
      const result = (await window.api.git.branches()) as unknown as {
        current: string;
        branches: Branch[];
      };
      set({ branches: result.branches });
    } catch (err) {
      logError("git", "Failed to load branches", err);
    }
  },
  ...
  switchBranch: async (name) => {
    try {
      ...
      logError("git", "Failed to switch branch", err);
    }
  },
  ...
  createBranch: async (name) => {
    try {
      ...
      logError("git", "Failed to create branch", err);
    }
  },
  ...
  deleteBranch: async (name) => {
    try {
      ...
      logError("git", "Failed to delete branch", err);
    }
  },
  ...
  loadHistory: async (limit = 50) => {
    try {
      ...
      logError("git", "Failed to load commit history", err);
    }
  },
  ...
  discardChanges: async (path) => {
    try {
      ...
      logError("git", "Failed to discard changes", err);
    }
  },
}));
