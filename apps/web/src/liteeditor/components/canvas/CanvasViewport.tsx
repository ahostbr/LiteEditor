...
export function useCanvasViewport() {
  ...
  useEffect(() => {
    ...
    const onMove = (e: PointerEvent) => {
      ...
      useCanvasStore
        .getState()
        .scrollTo(grabRef.current.startViewportX - dx, grabRef.current.startViewportY - dy);
      e.preventDefault();
      e.stopPropagation();
    };
    ...
    const onContext = (e: MouseEvent) => {
      if (grabRef.current.suppressContext) {
        ...
        setTimeout(() => {
          grabRef.current.suppressContext = false;
        }, 50);
      }
    };
  ...
  const springAnimate = useCallback(() => {
    ...
    if (
      Math.abs(vel.x) < SPRING_MIN_VELOCITY &&
      Math.abs(vel.y) < SPRING_MIN_VELOCITY &&
      Math.abs(dx) < 1 &&
      Math.abs(dy) < 1
    ) {
