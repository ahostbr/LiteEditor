...
export function AddPaneMenu({ show: externalShow, onClose, anchorX, anchorY }: AddPaneMenuProps) {
  ...
  const handleOpenFileInEditor = async () => {
    try {
      ...
      logWarn("AddPaneMenu", "File open cancelled or failed", err);
    }
    setInternalShow(false);
    onClose?.();
  };
  ...
  return (
    ...
    >
      ...
      <MenuItem
        ...
        label="Chat"
        onClick={() => addPaneOfType("chat")}
      />
      <MenuItem
        icon={<Terminal size={14} />}
        label="New Terminal"
        onClick={() => addPaneOfType("terminal")}
      />
      <MenuItem icon={<FileCode size={14} />} label="Open File" onClick={handleOpenFileInEditor} />
      <MenuItem
        icon={<FileCode size={14} />}
        label="Editor"
        onClick={() => addPaneOfType("unified-editor")}
      />
      <MenuItem
        icon={<GitBranch size={14} />}
        label="Git Panel"
        onClick={() => addPaneOfType("git")}
      />
      <MenuItem
        icon={<Globe size={14} />}
        label="New Browser"
        onClick={() => addPaneOfType("browser")}
      />
      <MenuItem
        icon={
          <img
      ...
      />
      <MenuItem icon={<FolderOpen size={14} />} label="Files" onClick={() => addPaneOfType("files")} />
      <MenuItem icon={<Search size={14} />} label="Search" onClick={() => addPaneOfType("search")} />
      <MenuItem icon={<Settings size={14} />} label="Settings" onClick={() => addPaneOfType("settings")} />
    </div>
  );
}
