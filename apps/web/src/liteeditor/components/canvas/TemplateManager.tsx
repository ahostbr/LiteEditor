...
export function TemplateManager() {
      ...
      for (const entry of list as any[]) {
        if (entry.name?.endsWith(".json")) {
          try {
            ...
            logError("TemplateManager", `Failed to parse template: ${entry.name}`, err);
          }
        }
      }
      setTemplates(loaded);
  ...
  function getTemplatesDir(): string {
    ...
    const homeDir =
      typeof window.api?.appInfo?.homeDir === "string" ? window.api.appInfo.homeDir : "";
    return `${homeDir || "~"}/.liteeditor/templates`;
  }
  ...
  async function deleteTemplate(template: TemplateData) {
    try {
      ...
      logError("TemplateManager", "Failed to delete template", err);
    }
  }
