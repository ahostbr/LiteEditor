...
export function SettingsPanel() {
  ...
  const [integrationProgress, setIntegrationProgress] = React.useState<
    Record<IntegrationId, IntegrationProgress | null>
  >({
    codex: null,
    claude: null,
  });
          ...
          <div className="space-y-6">
            <SettingsSection title="Integrations">
              ...
              <IntegrationCard
                ...
                onInstall={() =>
                  void runIntegrationAction("codex", () => window.api.integrations.install("codex"))
                }
                onUpdate={() =>
                  void runIntegrationAction("codex", () => window.api.integrations.update("codex"))
                }
                onVerify={() =>
                  void runIntegrationAction("codex", () => window.api.integrations.verify("codex"))
                }
                onReinstall={() =>
                  void runIntegrationAction("codex", () =>
                    window.api.integrations.install("codex", { reinstall: true }),
                  )
                }
              ...
              <IntegrationCard
                ...
                onInstall={() =>
                  void runIntegrationAction("claude", () =>
                    window.api.integrations.install("claude"),
                  )
                }
                onUpdate={() =>
                  void runIntegrationAction("claude", () =>
                    window.api.integrations.update("claude"),
                  )
                }
                onVerify={() =>
                  void runIntegrationAction("claude", () =>
                    window.api.integrations.verify("claude"),
                  )
                }
                onReinstall={() =>
                  void runIntegrationAction("claude", () =>
                    window.api.integrations.install("claude", { reinstall: true }),
                  )
                }
...
function IntegrationCard({
  ...
  const showReinstall =
    state === "installed_managed" || state === "update_available" || state === "broken";
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-wrap gap-1">
        {showInstall && <ActionButton label="Install" onClick={onInstall} disabled={busy} />}
        {showUpdate && <ActionButton label="Update" onClick={onUpdate} disabled={busy} />}
        {showVerify && <ActionButton label="Verify" onClick={onVerify} disabled={busy} />}
        {showReinstall && <ActionButton label="Reinstall" onClick={onReinstall} disabled={busy} />}
...
function InfoLine({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex items-center justify-between gap-2">
      ...
      <span className="truncate" style={{ color: "var(--text-primary)" }} title={value}>
        {value}
      </span>
    </div>
  );
  ...
  switch (state) {
    ...
    case "installed_external":
      return {
        ...
        border: "rgba(79,163,255,0.35)",
        text: "#9ccaff",
      };
    case "update_available":
      return {
        background: "rgba(255,174,66,0.1)",
        border: "rgba(255,174,66,0.35)",
        text: "#f7cb88",
      };
    case "broken":
    case "failed":
    ...
    case "verifying":
      return {
        background: "rgba(130,127,255,0.1)",
        border: "rgba(130,127,255,0.35)",
        text: "#c7c6ff",
      };
    default:
      return {
...
function ScopeSelector({
  ...
  currentScope,
  setScope,
}: ScopeProps & { currentScope: SettingScope; setScope: (s: SettingScope) => void }) {
