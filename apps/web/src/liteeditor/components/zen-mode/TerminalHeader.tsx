...
export function TerminalHeader({
  ...
  useEffect(() => {
    ...
    const handleClickOutside = (e: MouseEvent) => {
      if (
        cogMenuRef.current &&
        !cogMenuRef.current.contains(e.target as Node) &&
        cogButtonRef.current &&
        !cogButtonRef.current.contains(e.target as Node)
      ) {
  ...
  return (
    ...
    >
      <div className="flex items-center min-w-0 flex-1">
        ...
        {shellName && (
          <span
            className="ml-1.5 text-[11px] opacity-40 truncate"
        ...
        <button
          onClick={async (e) => {
            ...
            try {
              ...
              logWarn("TerminalHeader", "Clipboard paste failed", err);
            }
          }}
          className="p-1 rounded hover:bg-[var(--bg-muted)] opacity-50 hover:opacity-100 transition-opacity"
          title="Paste"
