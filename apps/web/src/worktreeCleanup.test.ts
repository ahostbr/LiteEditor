import { ProjectId, ThreadId } from "@liteeditor/contracts";
...
describe("formatWorktreePathForDisplay", () => {
  it("shows only the last path segment for unix-like paths", () => {
    const result = formatWorktreePathForDisplay(
      "/Users/julius/.liteeditor/ProjectData/worktrees/liteeditor-mvp/liteeditor-4e609bb8",
    );
    expect(result).toBe("liteeditor-4e609bb8");
  });

  it("normalizes windows separators before selecting the final segment", () => {
    const result = formatWorktreePathForDisplay(
      "C:\\Users\\julius\\.liteeditor\\ProjectData\\worktrees\\liteeditor-mvp\\liteeditor-4e609bb8",
    );
    expect(result).toBe("liteeditor-4e609bb8");
  });

  it("uses the final segment even when outside ~/.liteeditor/ProjectData/worktrees", () => {
