...
export default function LiteEditorThreadWorkspace(props: LiteEditorThreadWorkspaceProps) {
  ...
  useEffect(() => {
    ...
    if (savedState) {
      ...
      // Ensure all chat panes have the current threadId (re-get state after restore)
      const restored = useCanvasStore.getState();
      for (const [id, pane] of restored.panes) {
        if (pane.type === "chat") {
          restored.updatePane(id, { threadId: props.threadId });
        }
      }
    } else {
