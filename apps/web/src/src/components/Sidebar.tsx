...
function LiteEditorWordmark() {
  return (
    <svg
      aria-label="LiteEditor"
...
export default function Sidebar() {
        ...
        <TooltipTrigger
          render={
            <div className="flex min-w-0 flex-1 items-center gap-1.5 ml-1 cursor-pointer">
              ...
              />
              <span className="truncate text-sm font-medium tracking-tight text-foreground group-data-[state=collapsed]:hidden">
                LiteEditor
              </span>
  ...
  return (
    <>
      {isElectron ? (
        <>
          <SidebarHeader className={`h-8 flex-row items-center gap-1 px-2 py-0 ${isMacPlatform(navigator.platform) ? 'pl-[90px]' : ''}`}>
            <button
              type="button"
              onClick={() => toggleSidebar()}
              className="inline-flex size-6 shrink-0 items-center justify-center rounded-md text-muted-foreground/60 transition-colors hover:text-foreground hover:bg-muted/50"
              title={sidebarOpen ? "Collapse sidebar" : "Expand sidebar"}
            >
              {sidebarOpen ? <ChevronLeftIcon className="size-3.5" /> : <ChevronRightIcon className="size-3.5" />}
            </button>
            <div className="group-data-[state=collapsed]:hidden flex-1 min-w-0">
              {wordmark}
            </div>
        ...
        <SidebarHeader className="gap-3 px-3 py-0 h-8 sm:gap-2.5 sm:px-4">
          {wordmark}
        </SidebarHeader>
      )}
