...
export function getMonacoContent(filePath: string): string | undefined {
  try {
    ...
    logWarn("editor", "Failed to get Monaco content", err);
    return undefined;
  }
}
...
function disposeMonacoModel(key: string, allPanes: [PaneState, PaneState | null]): void {
  ...
  try {
    ...
    logWarn("editor", "Failed to dispose Monaco model", err);
  }
}
...
export const useEditorStore = create<EditorState>((set, get) => ({
  ...
  closeTab: (paneIndex, tabIndex) => {
    set((state) => {
      ...
      if (paneIndex === 1 && newTabs.length === 0) {
        const result = {
          panes: [newPanes[0]!, null] as [PaneState, PaneState | null],
          isSplit: false,
          activePaneIndex: 0 as const,
        };
  ...
  openDiff: (path, original, modified, paneIndex) => {
    set((state) => {
      ...
      const existingIndex = pane.tabs.findIndex((t) => t.type === "diff" && t.path === path);
      if (existingIndex >= 0) {
        const newPanes = [...state.panes] as [PaneState, PaneState | null];
        const newTabs = [...pane.tabs];
      ...
      for (const tab of pane.tabs) {
        if (tab.path) {
          try {
            ...
            logWarn("editor", "Failed to dispose Monaco model during workspace restore", err);
          }
        }
      }
    }
