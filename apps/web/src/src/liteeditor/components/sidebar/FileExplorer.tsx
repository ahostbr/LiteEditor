...
export function FileExplorer() {
  ...
  const handleOpenFolder = async () => {
    ...
    if (path) {
      setProjectRoot(path);
      try {
        await window.api.git.init(path);
      } catch (err) {
        logWarn("FileExplorer", "Git init skipped (not a git repo)", err);
      }
      try {
        await window.api.search.setRoot(path);
      } catch (err) {
        logWarn("FileExplorer", "Failed to set search root", err);
      }
    }
  };
  ...
  const handleFileClick = async (node: FileNode) => {
    ...
    try {
      ...
      logError("FileExplorer", `Failed to open file: ${node.path}`, err);
    }
  };
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      >
        <span
          className="text-[10px] font-mono font-semibold uppercase tracking-widest opacity-80"
