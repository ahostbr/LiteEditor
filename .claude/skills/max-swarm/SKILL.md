...
Spawn multiple coding agents in parallel to work on a task. Each agent works on independent files or modules simultaneously.
...
For each subtask, spawn a Claude agent using the `Agent` tool:
- `run_in_background: true` (required — runs in background, you get notified when done)
- `subagent_type: "do-work-builder"` or `"general-purpose"` depending on task
- `mode: "bypassPermissions"` for unattended, `"default"` otherwise

**CRITICAL: Spawn ALL agents in a SINGLE message with parallel tool calls!**

```
// DO THIS - all in one message:
Agent({ prompt: "Task 1 instructions...", description: "task-1", run_in_background: true })
Agent({ prompt: "Task 2 instructions...", description: "task-2", run_in_background: true })
Agent({ prompt: "Task 3 instructions...", description: "task-3", run_in_background: true })
```
...
**User:** `/max-swarm "Write comprehensive documentation for the project"`
...
[Spawn 4 agents in parallel with Agent tool...]
