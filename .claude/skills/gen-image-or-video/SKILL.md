...
Two tiers: **1) Gemini API** (default, Python script) → **2) LiteImage** (local SD).
...
GEN="python ~/.claude/skills/gen-image-or-video/gen_image.py"
...
VEO="python ~/.claude/skills/gen-image-or-video/gen_video.py"
...
3. **Image:** run `gen_image.py` (Gemini default) or LiteImage fallback
4. **Video:** run `gen_video.py` (Veo default)
5. Review output (use Read tool on saved path)
6. Post-process if needed (frame extraction, compression)
7. Generate 2-3 variants for A/B testing when doing marketing work
