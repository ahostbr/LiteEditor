...
1. **Start LiteWatch** if it's not already running:
   ```bash
   start "" "~/.claude/skills/local-lens/LiteWatch.bat"
   ```
...
2. **Ensure a VLM is loaded** in LM Studio (configure your preferred vision model in LM Studio — a small, fast VLM works best):
   ```bash
   "$USERPROFILE/.lmstudio/bin/lms.exe" ps 2>&1
   ```
   If no VLM is loaded, load one:
   ```bash
   "$USERPROFILE/.lmstudio/bin/lms.exe" load "your-vlm-model-id" -y -c 8192 --gpu max
   ```

3. **Check screen context** by reading:
   ```
   ~/.claude/skills/local-lens/screen_context.txt
   ```
...
```
Read ~/.claude/skills/local-lens/screen_context.txt
```
...
- If entries show `ERROR: HTTP Error 400`, a non-VLM model is loaded. Load a vision-capable model.
