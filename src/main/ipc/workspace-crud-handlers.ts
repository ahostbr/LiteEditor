...
export function registerWorkspaceCrudHandlers(): void {
  ...
  ipcMain.handle('workspaces:create', async (_e, projectId: string, name: string, type?: string, branch?: string, worktreePath?: string) => {
  ...
  ipcMain.handle('workspaces:rename', async (_e, projectId: string, workspaceId: string, name: string) => {
