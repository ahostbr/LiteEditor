...
export class ClaudeBridge {
  ...
  private sendInitResponse(wc: WebContents, requestId: string): void {
    ...
    // Build auth status from credentials file
    const hasAuth = !!(creds?.claudeAiOauth?.accessToken)
  ...
  private sendClaudeStateResponse(wc: WebContents, requestId: string): void {
    const creds = this.readCredentials()
    const hasAuth = !!(creds?.claudeAiOauth?.accessToken)
  ...
  private async sendSessionsList(wc: WebContents, requestId: string): Promise<void> {
    try {
  ...
  private async sendSessionMessages(wc: WebContents, requestId: string, sessionId: string): Promise<void> {
    try {
      const homeDir = process.env.USERPROFILE || process.env.HOME || ''
      const projectsDir = join(homeDir, '.claude', 'projects')

      if (!existsSync(projectsDir)) {
        this.sendToWebview(wc, { type: 'response', requestId, response: { messages: [], sessionDiffs: [] } })
        return
      }
      ...
      this.sendToWebview(wc, {
        type: 'response',
        requestId,
        response: { messages, sessionDiffs: [] }
      })
    } catch {
      this.sendToWebview(wc, { type: 'response', requestId, response: { messages: [], sessionDiffs: [] } })
    }
  ...
  private sendUsageResponse(wc: WebContents, requestId: string): void {
    // Send a usage_update message (this is an async push, not a response)
    this.sendToWebview(wc, {
      type: 'request',
      request: {
        type: 'usage_update',
    ...
    // Also send the response to the request to avoid hanging
    this.sendToWebview(wc, {
      type: 'response',
      requestId,
      response: {}
