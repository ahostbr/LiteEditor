import { INTEGRATION_SPECS, type IntegrationId, type IntegrationManifest } from './integrations-types'
...
export class MarketplaceProvider {
  async fetchLatestManifest(id: IntegrationId): Promise<IntegrationManifest> {
    ...
    if (publisher !== spec.publisher || name !== spec.name) {
      throw new Error(`Marketplace identity mismatch for ${spec.extensionId}: got ${publisher}.${name}`)
    }
