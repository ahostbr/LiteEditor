...
function reviewBadge(decision: PullRequest['reviewDecision']) {
  switch (decision) {
    case 'APPROVED':
      return <span className="text-[8px] px-1 rounded font-medium" style={{ backgroundColor: 'rgba(34,197,94,0.15)', color: '#22c55e' }}>Approved</span>
    case 'CHANGES_REQUESTED':
      return <span className="text-[8px] px-1 rounded font-medium" style={{ backgroundColor: 'rgba(239,68,68,0.15)', color: '#ef4444' }}>Changes</span>
    case 'REVIEW_REQUIRED':
      return <span className="text-[8px] px-1 rounded font-medium" style={{ backgroundColor: 'rgba(234,179,8,0.15)', color: '#eab308' }}>Review</span>
    default: return null
  }
}
...
export function PullRequestsPanel({ onCreatePr }: PullRequestsPanelProps) {
  ...
  const handleFilterChange = useCallback((filter: PrFilter) => {
  ...
  return (
    <div className="flex flex-col h-full">
      {/* Header with filters */}
      <div className="flex items-center gap-1 px-2 py-1 shrink-0" style={{ borderBottom: '1px solid var(--border)' }}>
        ...
        {pullRequests.map((pr) => (
          ...
          >
            <div className="flex items-center gap-1.5">
              <GitPullRequest size={12} style={{
                color: pr.state === 'MERGED' ? '#a855f7' : pr.state === 'CLOSED' ? '#ef4444' : '#22c55e',
                flexShrink: 0
              }} />
            ...
            <div className="flex items-center gap-1.5 mt-0.5 ml-[18px]">
              ...
              {pr.isDraft && (
                <span className="text-[8px] px-1 rounded" style={{ backgroundColor: 'var(--bg-overlay)', color: 'var(--text-muted)' }}>Draft</span>
              )}
