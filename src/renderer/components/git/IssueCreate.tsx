  ...
  return (
    <div className="flex flex-col h-full">
      <div className="flex items-center justify-between px-3 py-2 shrink-0" style={{ borderBottom: '1px solid var(--border)' }}>
        <span className="text-[11px] font-semibold" style={{ color: 'var(--text-primary)' }}>Create Issue</span>
      ...
      <div className="flex-1 overflow-y-auto px-3 py-2 space-y-2">
        ...
        />
        {error && (
          <div className="text-[10px] px-2 py-1 rounded" style={{ color: '#ef4444', backgroundColor: 'rgba(239,68,68,0.1)' }}>
            {error}
          </div>
        )}
      </div>
      <div className="flex justify-end gap-2 px-3 py-2 shrink-0" style={{ borderTop: '1px solid var(--border)' }}>
        <button onClick={onClose} className="px-3 py-1.5 rounded text-[11px] font-medium" style={{ color: 'var(--text-secondary)', backgroundColor: 'var(--bg-overlay)' }}>
          Cancel
        </button>
        <button
