...
function lineColor(type: DiffLine['type']): string {
  switch (type) {
    case 'add': return 'rgba(34, 197, 94, 0.12)'
    case 'delete': return 'rgba(239, 68, 68, 0.12)'
    case 'hunk': return 'rgba(96, 165, 250, 0.08)'
...
function lineTextColor(type: DiffLine['type']): string {
  switch (type) {
    ...
    case 'hunk': return 'var(--text-muted)'
  ...
  return (
    <div className="h-full overflow-auto font-mono text-[12px] leading-[18px]">
      ...
      <div
        ...
        style={{ backgroundColor: 'var(--bg-surface)', borderBottom: '1px solid var(--border)', color: 'var(--text-secondary)', zIndex: 1 }}
      >
        {diff.path}
        <span className="ml-3" style={{ color: '#22c55e' }}>+{diff.additions}</span>
        <span className="ml-2" style={{ color: '#ef4444' }}>-{diff.deletions}</span>
      </div>
          ...
          {hunk.lines.map((line, li) => (
            ...
            >
              ...
              <span
                ...
                style={{ color: line.type === 'hunk' ? 'var(--text-muted)' : 'var(--text-primary)' }}
              >
                {line.content}
              </span>
            </div>
