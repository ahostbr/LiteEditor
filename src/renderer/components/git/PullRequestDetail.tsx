...
export function PullRequestDetail() {
  ...
  const handleSubmitReview = async (event: 'APPROVE' | 'REQUEST_CHANGES' | 'COMMENT') => {
    ...
    const result = await useDialogStore.getState().showDialog({
      ...
      detail: pendingReviewComments.length > 0 ? `${pendingReviewComments.length} inline comment(s) will be included.` : undefined,
      buttons: [
        { label: labels[event], variant: event === 'APPROVE' ? 'primary' : event === 'REQUEST_CHANGES' ? 'destructive' : 'muted' },
        { label: 'Cancel', variant: 'muted' }
      ],
      cancelIndex: 1
    })
  ...
  const stateColor = prDetail.state === 'MERGED' ? '#a855f7' : prDetail.state === 'CLOSED' ? '#ef4444' : '#22c55e'
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="px-3 py-2 shrink-0" style={{ borderBottom: '1px solid var(--border)' }}>
        <div className="flex items-center gap-2">
          ...
          <span className="text-[9px] px-1.5 rounded font-medium" style={{ backgroundColor: `${stateColor}20`, color: stateColor }}>
            {prDetail.state}
          </span>
          {prDetail.isDraft && (
            <span className="text-[9px] px-1.5 rounded" style={{ backgroundColor: 'var(--bg-overlay)', color: 'var(--text-muted)' }}>Draft</span>
          )}
        </div>
        <div className="flex items-center gap-2 mt-1 text-[10px]" style={{ color: 'var(--text-muted)' }}>
          ...
          <span className="ml-auto" style={{ color: '#22c55e' }}>+{prDetail.additions}</span>
          ...
          <div className="flex items-center gap-1.5 mt-2">
            ...
            >
              ...
              {pendingReviewComments.length > 0 && (
                <span className="text-[8px] px-1 rounded-full" style={{ backgroundColor: 'var(--accent)', color: '#000' }}>
        ...
        {showReviewForm && (
          <div className="mt-2 p-2 rounded" style={{ backgroundColor: 'var(--bg-muted)', border: '1px solid var(--border)' }}>
            <textarea
              value={reviewBody}
      ...
      {prDiff && prDiff.length > 0 && (
        <div className="flex flex-col flex-1 min-h-0">
          {/* File tabs */}
          <div className="flex gap-0 overflow-x-auto shrink-0" style={{ borderBottom: '1px solid var(--border)' }}>
            {prDiff.map((file, i) => (
              <button
                key={file.path}
                ...
                style={{
                  ...
                  borderBottom: i === selectedFileIndex ? '2px solid var(--accent)' : '2px solid transparent'
                }}
              >
                {file.path.replace(/^.*[\\/]/, '')}
              </button>
