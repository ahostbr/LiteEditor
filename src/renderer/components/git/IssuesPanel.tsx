...
export function IssuesPanel({ onCreateIssue }: IssuesPanelProps) {
  ...
  const handleFilterChange = useCallback((filter: IssueFilter) => {
    loadIssues(filter)
  }, [loadIssues])
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="flex items-center gap-1 px-2 py-1 shrink-0" style={{ borderBottom: '1px solid var(--border)' }}>
        ...
        <button onClick={onCreateIssue} className="p-0.5 rounded" style={{ color: 'var(--text-muted)' }} title="New Issue">
          <Plus size={12} />
        </button>
        <button onClick={() => loadIssues()} className="p-0.5 rounded" style={{ color: 'var(--text-muted)' }} title="Refresh">
      ...
      <div className="flex-1 overflow-y-auto">
        {issues.map((issue) => (
          <div
            ...
            style={{
              backgroundColor: issue.number === selectedIssueNumber ? 'var(--bg-overlay)' : undefined,
              borderColor: 'var(--border)'
            }}
            onClick={() => selectIssue(issue.number === selectedIssueNumber ? null : issue.number)}
          >
            <div className="flex items-center gap-1.5">
              <CircleDot size={12} style={{
            ...
            <div className="flex items-center gap-1.5 mt-0.5 ml-[18px]">
              <span className="text-[10px]" style={{ color: 'var(--text-muted)' }}>{issue.author.login}</span>
