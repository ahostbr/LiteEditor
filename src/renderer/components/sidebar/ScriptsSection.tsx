...
import { ChevronRight, ChevronDown, Play, Square, Hammer, FlaskConical, CheckCircle, Settings, Bug, Rocket, Terminal, Globe } from 'lucide-react'
...
export function ScriptsSection({ projectId, projectRootPath }: ScriptsSectionProps) {
  ...
  const isRunning = useCallback((scriptId: string) => {
  ...
  const stopScript = useCallback((script: DetectedScript) => {
    ...
    <div>
      ...
      >
        ...
        {!expanded && runningDevCount > 0 && (
          <StatusDot state="running" size="sm" title={`${runningDevCount} running`} className="ml-auto" />
        )}
      </button>
      ...
      {expanded && (
        <div>
          {loading ? (
            <div className="text-[9px] py-1" style={{ paddingLeft: '32px', color: 'var(--text-muted)' }}>Detecting...</div>
          ) : scripts.length === 0 ? (
            <div className="text-[9px] py-1" style={{ paddingLeft: '32px', color: 'var(--text-muted)' }}>No scripts found</div>
          ) : (
              ...
              return (
                ...
                >
                  ...
                  >
                    {script.name}
                    {isDev && !running && (
                      <span className="text-[8px] ml-1 opacity-40">dev</span>
                    )}
                  </span>
