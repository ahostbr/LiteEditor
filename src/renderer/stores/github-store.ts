...
interface GitHubState {
  ...
  createPullRequest: (title: string, body: string, base: string, head: string, reviewers?: string[], labels?: string[]) => Promise<PullRequest>
  ...
  createIssue: (title: string, body: string, labels?: string[], assignees?: string[]) => Promise<Issue>
...
export const useGitHubStore = create<GitHubState>((set, get) => ({
  ...
  checkGhCli: async () => {
    try {
      const result = (await window.api.github.checkCli()) as { installed: boolean; authenticated: boolean }
      set({
  ...
  selectPullRequest: async (number) => {
    if (number === null) {
      set({ selectedPrNumber: null, prDetail: null, prDiff: null, prReviews: [], prReviewComments: [] })
      return
    }
  ...
  createPullRequest: async (title, body, base, head, reviewers, labels) => {
    const pr = (await window.api.github.prCreate(title, body, base, head, reviewers, labels)) as PullRequest
  ...
  submitReview: async (prNumber, event, body) => {
    ...
    const comments = pendingReviewComments.length > 0
