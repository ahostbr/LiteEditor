...
export function useCanvasNavigation() {
  const handleCanvasKey = useCallback((e: KeyboardEvent) => {
    ...
    if (ctrl && shift && ['ArrowLeft', 'ArrowRight', 'ArrowUp', 'ArrowDown'].includes(e.key)) {
      ...
      const step = 20
      switch (e.key) {
        case 'ArrowLeft': store.movePane(focused.id, focused.x - step, focused.y); break
        case 'ArrowRight': store.movePane(focused.id, focused.x + step, focused.y); break
        case 'ArrowUp': store.movePane(focused.id, focused.x, focused.y - step); break
        case 'ArrowDown': store.movePane(focused.id, focused.x, focused.y + step); break
      }
      return
    }
