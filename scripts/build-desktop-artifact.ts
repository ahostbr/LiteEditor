...
interface StagePackageJson {
  ...
  readonly liteeditorCommitHash: string;
...
const BuildEnvConfig = Config.all({
  platform: Config.schema(BuildPlatform, "LITEEDITOR_DESKTOP_PLATFORM").pipe(Config.option),
  target: Config.string("LITEEDITOR_DESKTOP_TARGET").pipe(Config.option),
  arch: Config.schema(BuildArch, "LITEEDITOR_DESKTOP_ARCH").pipe(Config.option),
  version: Config.string("LITEEDITOR_DESKTOP_VERSION").pipe(Config.option),
  outputDir: Config.string("LITEEDITOR_DESKTOP_OUTPUT_DIR").pipe(Config.option),
  skipBuild: Config.boolean("LITEEDITOR_DESKTOP_SKIP_BUILD").pipe(Config.withDefault(false)),
  keepStage: Config.boolean("LITEEDITOR_DESKTOP_KEEP_STAGE").pipe(Config.withDefault(false)),
  signed: Config.boolean("LITEEDITOR_DESKTOP_SIGNED").pipe(Config.withDefault(false)),
  verbose: Config.boolean("LITEEDITOR_DESKTOP_VERBOSE").pipe(Config.withDefault(false)),
});
...
function resolveGitHubPublishConfig():
  ...
  | undefined {
  const rawRepo =
    process.env.LITEEDITOR_DESKTOP_UPDATE_REPOSITORY?.trim() ||
...
const createBuildConfig = Effect.fn("createBuildConfig")(function* (
  ...
  const buildConfig: Record<string, unknown> = {
    appId: "com.litesuite.liteeditor",
    productName,
    executableName: "liteeditor",
    artifactName: "liteeditor-${version}-${arch}.${ext}",
    directories: {
...
const buildDesktopArtifact = Effect.fn("buildDesktopArtifact")(function* (
  ...
  const stageRoot = yield* mkdir({
    prefix: `liteeditor-desktop-${options.platform}-stage-`,
  });
  ...
  const stagePackageJson: StagePackageJson = {
    ...
    liteeditorCommitHash: commitHash,
    private: true,
    description: "LiteEditor desktop build",
    author: "Lite Suite",
    main: "apps/desktop/dist-electron/main.js",
    build: yield* createBuildConfig(
      options.platform,
      options.target,
      desktopPackageJson.productName ?? "LiteEditor",
      options.signed,
    ),
    dependencies: {
...
const buildDesktopArtifactCli = Command.make("build-desktop-artifact", {
  platform: Flag.choice("platform", BuildPlatform.literals).pipe(
    Flag.withDescription("Build platform (env: LITEEDITOR_DESKTOP_PLATFORM)."),
    Flag.optional,
  ),
  target: Flag.string("target").pipe(
    Flag.withDescription(
      "Artifact target, for example dmg/AppImage/nsis (env: LITEEDITOR_DESKTOP_TARGET).",
    ),
    Flag.optional,
  ),
  arch: Flag.choice("arch", BuildArch.literals).pipe(
    Flag.withDescription("Build arch, for example arm64/x64/universal (env: LITEEDITOR_DESKTOP_ARCH)."),
    Flag.optional,
  ),
  buildVersion: Flag.string("build-version").pipe(
    Flag.withDescription("Artifact version metadata (env: LITEEDITOR_DESKTOP_VERSION)."),
    Flag.optional,
  ),
  outputDir: Flag.string("output-dir").pipe(
    Flag.withDescription("Output directory for artifacts (env: LITEEDITOR_DESKTOP_OUTPUT_DIR)."),
    Flag.optional,
  ),
  skipBuild: Flag.boolean("skip-build").pipe(
    Flag.withDescription(
      "Skip `bun run build:desktop` and use existing dist artifacts (env: LITEEDITOR_DESKTOP_SKIP_BUILD).",
    ),
    Flag.optional,
  ),
  keepStage: Flag.boolean("keep-stage").pipe(
    Flag.withDescription("Keep temporary staging files (env: LITEEDITOR_DESKTOP_KEEP_STAGE)."),
    Flag.optional,
  ),
  signed: Flag.boolean("signed").pipe(
    Flag.withDescription(
      "Enable signing/notarization discovery; Windows uses Azure Trusted Signing (env: LITEEDITOR_DESKTOP_SIGNED).",
    ),
    Flag.optional,
  ),
  verbose: Flag.boolean("verbose").pipe(
    Flag.withDescription("Stream subprocess stdout (env: LITEEDITOR_DESKTOP_VERBOSE)."),
    Flag.optional,
  ),
}).pipe(
  Command.withDescription("Build a desktop artifact for LiteEditor."),
