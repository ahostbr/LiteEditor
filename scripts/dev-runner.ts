...
export const DEFAULT_LITEEDITOR_HOME = Effect.map(Effect.service(Path.Path), (path) =>
  path.join(homedir(), ".liteeditor", "ProjectData"),
);
...
const MODE_ARGS = {
  dev: [
    ...
    "dev",
    "--ui=tui",
    "--filter=@liteeditor/contracts",
    "--filter=@liteeditor/web",
    "--filter=liteeditor-server",
    "--parallel",
  ],
  "dev:server": ["run", "dev", "--filter=liteeditor-server"],
  "dev:web": ["run", "dev", "--filter=@liteeditor/web"],
  "dev:desktop": ["run", "dev", "--filter=@liteeditor/desktop", "--filter=@liteeditor/web", "--parallel"],
...
const OffsetConfig = Config.all({
  portOffset: optionalIntegerConfig("LITEEDITOR_PORT_OFFSET"),
  devInstance: optionalStringConfig("LITEEDITOR_DEV_INSTANCE"),
});
...
export function resolveOffset(config: {
  ...
  if (config.portOffset !== undefined) {
    if (config.portOffset < 0) {
      throw new Error(`Invalid LITEEDITOR_PORT_OFFSET: ${config.portOffset}`);
    }
    return {
      offset: config.portOffset,
      source: `LITEEDITOR_PORT_OFFSET=${config.portOffset}`,
    };
  }
  ...
  if (/^\d+$/.test(seed)) {
    return { offset: Number(seed), source: `numeric LITEEDITOR_DEV_INSTANCE=${seed}` };
  }

  const offset = ((Hash.string(seed) >>> 0) % MAX_HASH_OFFSET) + 1;
  return { offset, source: `hashed LITEEDITOR_DEV_INSTANCE=${seed}` };
}
...
function resolveBaseDir(baseDir: string | undefined): Effect.Effect<string, never, Path.Path> {
  return Effect.gen(function* () {
    ...
    return yield* DEFAULT_LITEEDITOR_HOME;
  });
}
...
export function createDevRunnerEnv({
  ...
  return Effect.gen(function* () {
    ...
    const output: NodeJS.ProcessEnv = {
      ...baseEnv,
      LITEEDITOR_PORT: String(serverPort),
      PORT: String(webPort),
      ELECTRON_RENDERER_PORT: String(webPort),
      VITE_WS_URL: `ws://localhost:${serverPort}`,
      VITE_DEV_SERVER_URL: devUrl?.toString() ?? `http://localhost:${webPort}`,
      LITEEDITOR_HOME: resolvedBaseDir,
    };

    if (host !== undefined) {
      output.LITEEDITOR_HOST = host;
    }

    if (authToken !== undefined) {
      output.LITEEDITOR_AUTH_TOKEN = authToken;
    } else {
      delete output.LITEEDITOR_AUTH_TOKEN;
    }

    if (noBrowser !== undefined) {
      output.LITEEDITOR_NO_BROWSER = noBrowser ? "1" : "0";
    } else {
      delete output.LITEEDITOR_NO_BROWSER;
    }

    if (autoBootstrapProjectFromCwd !== undefined) {
      output.LITEEDITOR_AUTO_BOOTSTRAP_PROJECT_FROM_CWD = autoBootstrapProjectFromCwd ? "1" : "0";
    } else {
      delete output.LITEEDITOR_AUTO_BOOTSTRAP_PROJECT_FROM_CWD;
    }

    if (logWebSocketEvents !== undefined) {
      output.LITEEDITOR_LOG_WS_EVENTS = logWebSocketEvents ? "1" : "0";
    } else {
      delete output.LITEEDITOR_LOG_WS_EVENTS;
    }

    if (mode === "dev") {
      output.LITEEDITOR_MODE = "web";
      delete output.LITEEDITOR_DESKTOP_WS_URL;
    }
  ...
  return Effect.gen(function* () {
    const { portOffset, devInstance } = yield* OffsetConfig.asEffect().pipe(
      Effect.mapError(
        (cause) =>
          new DevRunnerError({
            message: "Failed to read LITEEDITOR_PORT_OFFSET/LITEEDITOR_DEV_INSTANCE configuration.",
            cause,
          }),
      ),
    );
    ...
    const envOverrides = {
      noBrowser: readOptionalBooleanEnv("LITEEDITOR_NO_BROWSER"),
      autoBootstrapProjectFromCwd: readOptionalBooleanEnv("LITEEDITOR_AUTO_BOOTSTRAP_PROJECT_FROM_CWD"),
      logWebSocketEvents: readOptionalBooleanEnv("LITEEDITOR_LOG_WS_EVENTS"),
    };
    ...
    yield* Effect.logInfo(
      `[dev-runner] mode=${input.mode} source=${source}${selectionSuffix} serverPort=${String(env.LITEEDITOR_PORT)} webPort=${String(env.PORT)} baseDir=${String(env.LITEEDITOR_HOME)}`,
    );
...
const devRunnerCli = Command.make("dev-runner", {
  ...
  homeDir: Flag.string("home-dir").pipe(
    ...
    Flag.withFallbackConfig(optionalStringConfig("LITEEDITOR_HOME")),
  ),
  authToken: Flag.string("auth-token").pipe(
    Flag.withDescription("Auth token (forwards to LITEEDITOR_AUTH_TOKEN)."),
    Flag.withAlias("token"),
    Flag.withFallbackConfig(optionalStringConfig("LITEEDITOR_AUTH_TOKEN")),
  ),
  noBrowser: Flag.boolean("no-browser").pipe(
    Flag.withDescription("Browser auto-open toggle (equivalent to LITEEDITOR_NO_BROWSER)."),
    Flag.withFallbackConfig(optionalBooleanConfig("LITEEDITOR_NO_BROWSER")),
  ),
  autoBootstrapProjectFromCwd: Flag.boolean("auto-bootstrap-project-from-cwd").pipe(
    Flag.withDescription(
      "Auto-bootstrap toggle (equivalent to LITEEDITOR_AUTO_BOOTSTRAP_PROJECT_FROM_CWD).",
    ),
    Flag.withFallbackConfig(optionalBooleanConfig("LITEEDITOR_AUTO_BOOTSTRAP_PROJECT_FROM_CWD")),
  ),
  logWebSocketEvents: Flag.boolean("log-websocket-events").pipe(
    Flag.withDescription("WebSocket event logging toggle (equivalent to LITEEDITOR_LOG_WS_EVENTS)."),
    Flag.withAlias("log-ws-events"),
    Flag.withFallbackConfig(optionalBooleanConfig("LITEEDITOR_LOG_WS_EVENTS")),
  ),
  host: Flag.string("host").pipe(
    Flag.withDescription("Server host/interface override (forwards to LITEEDITOR_HOST)."),
    Flag.withFallbackConfig(optionalStringConfig("LITEEDITOR_HOST")),
  ),
  port: Flag.integer("port").pipe(
    Flag.withSchema(Schema.Int.check(Schema.isBetween({ minimum: 1, maximum: 65535 }))),
    Flag.withDescription("Server port override (forwards to LITEEDITOR_PORT)."),
    Flag.withFallbackConfig(optionalPortConfig("LITEEDITOR_PORT")),
  ),
